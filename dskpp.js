	...
	if (req.is("application/dskpp+xml")) {
		...
		req.body = xmlParser.parseString(msg, function(err, result) {
			...
			if (req.body["dskpp:KeyProvClientHello"]) {
				...
				db.sessions.insert({"timestamp":Date.now()},function(error,body){
					if (!error && body.ok) {
						...
						res.render(__dirname + "/views/keyprov_server_hello",{"sessionId":body.id,"serverNonce":serverNonce},function(err, xml){
							...
							db.sessions.update({'type':'session','r_s':serverNonce,'timestamp':Date.now(),'ip_address':req.ip,'messages':msg+xml,'device_manufacturer':deviceManufacturer,'device_serial_no':deviceSerialNo,'_rev':body.rev,'_id':body.id}, function(err) {
								if (!err) {
				...
				db.sessions.get(sessionId, function(error, body) {
										...
										if (mac.equals(authDataMac)) {
											...
											cryptoModule.serverPublicKey(hostname, function(err, serverPublicKey) {
												...
												function saveKey(keyObject) {
													...
													db.keys.insert(keyObject, function(err, body) {
														if (err) {
															res.sendStatus(500);
															return;
														}
														...
														db.clients.update(client, function() {
															var keyId = body.id;
													...
													db.sessions.destroy({"id":sessionId,"rev":rev});
												}
												// Get the keys registered for the device.
												db.keys.list({"device_id":deviceSerialNo}, function(err, result) {
													...
													if (!err && result.length > 0) {
														for (i in result) {
															...
															if (doc.value.client_id == clientId) {
																...
																db.keys.get(doc.id, function(err, key) {
																	if (!err) {
